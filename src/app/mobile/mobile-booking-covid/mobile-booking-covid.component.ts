...
@Component({
  selector: 'app-mobile-booking-covid',
  templateUrl: './mobile-booking-covid.component.html',
  styleUrls: ['./mobile-booking-covid.component.css']
})
export class MobileBookingCovidComponent extends CoreComponent implements OnInit {
  ...
  if (result === false) {
        this.entriesService.save(this.item).subscribe((data: Entry) => {
          this.item = data;
          that.router.navigate(['/mobile-select-date/' + that.service.id], {
            queryParams: {
              id: that.item.user.id,
              name: that.item.user.firstName,
              contactNumber: that.item.user.contactNumber,
              email: that.item.user.email,
              idNumber: that.item.user.idNumber,
              entryId: that.item.id,
            },
          });
          this.blockUI.stop();
